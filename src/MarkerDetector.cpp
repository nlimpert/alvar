...
void MarkerDetectorImpl::TrackMarkersReset() { _track_markers_clear(); }
...
int MarkerDetectorImpl::Detect(IplImage *image, Camera *cam, bool track,
  ...
  if (track) {
    for (size_t ii = 0; ii < _track_markers_size(); ii++) {
      ...
      if (track_error <= max_track_error) {
        ...
        if (visualize)
          mn->Visualize(image, cam, cvScalar(CV_RGB(255, 255, 0)));
      }
    }
  }
  ...
  for (size_t i = 0; i < blob_corners.size(); ++i) {
    ...
    if (mn->UpdateContent(blob_corners[i], gray, cam) &&
      ...
      if (visualize)
        mn->Visualize(image, cam, cvScalar(CV_RGB(255, 0, 0)));
    }
    delete mn;
  }
