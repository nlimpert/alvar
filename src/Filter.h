 ...
 *		slope = ((1.0-gamma) * (slope)) + (gamma * (value -
 *value_prev)); \endcode If the weight values (\e alpha , \e gamma) are larger
 *(near 1.0) the formulas react faster for changes and if they are near 0.0 then
 *the reaction is slower. The weight values \e alpha and \e gamma may be set in
 *the constructor or with \e setAlpha() and \e setGamma() .
 */
 ...
 *
 */
template <class F> class ALVAR_EXPORT FilterArray {
protected:
  double *tmp;
  std::vector<F> arr;
