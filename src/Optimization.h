...
public:
   ...
   * measurements. \param state		System parameters, e.g. camera
   * parameterization in optical tracking. \param projection	The system state
   * projection is stored here. E.g image measurements in optical tracking.
   * \param param		Additional parameters to the function. E.g. some constant
   ...
   * \param x		The set of parameters around which the Jacobian is
   * evaluated. \param J		Resulting Jacobian matrix is stored
   * here. \param Estimate	The function to be differentiated.
   */
   ...
   * observed.
   * \param stop				Optimization loop ends as the \e stop limit is
   * reached. Criteria is calculated as \param max_iter
   * Maximum number of iteration loops that are evaluated if \e stop is not
   * reached.
   * \param Estimate			Pointer to the function that maps the state to the
   * measurements. See \e EstimateCallback.
   * \param method			One of the three possible optimization
   * methods. \param parameters_mask	Vector that defines the parameters that
   * are optimized. If vector element is 0, corresponding parameter is not
   * altered.
   * \param J_mat			Jacobian matrix. If not given, numerical
   * differentation is used.
   * \param weights			Weight vector that can be submitted to give
   * different weights to different measurements. Currently works only with
