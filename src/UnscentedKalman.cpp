...
UnscentedKalman::UnscentedKalman(int state_n, int obs_n, int state_k,
...
void UnscentedKalman::initialize() {
  ...
  for (int i = 0; i < 5; i++)
    printf(
        ...
        cvGetReal2D(stateCovariance, 2, i), cvGetReal2D(stateCovariance, 3, i),
  ...
  for (int i = 0; i < 5; i++)
    printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(stateTmp, 0, i),
           cvGetReal2D(stateTmp, 1, i), cvGetReal2D(stateTmp, 2, i),
...
void UnscentedKalman::predict(UnscentedProcess *process_model) {
  ...
  for (int i = 0; i < sigma_n; i++) {
    double weight =
        i == 0 ? lambda / (L + lambda) + lambda2 : .5 / (L + lambda);
    totalWeight += weight;
  }
  for (int i = 0; i < sigma_n; i++) {
    double weight =
...
void UnscentedKalman::update(UnscentedObservation *obs) {
  ...
  for (int i = 0; i < sigma_n; i++) {
    ...
    double scale = i == 0 ? (double)state_k / (double)(state_n + state_k)
  ...
  CvMat predObsCovariance =
      cvMat(obs_n, obs_n, CV_64F, this->predObsCovariance->data.db);
  CvMat statePredObsCrossCorrelation = cvMat(
      state_n, obs_n, CV_64F, this->statePredObsCrossCorrelation->data.db);
  ...
  for (int i = 0; i < sigma_n; i++) {
    ...
    cvGEMM(&predObsDiff, &predObsDiff, scale, &predObsCovariance, 1.,
           &predObsCovariance, CV_GEMM_B_T);
  ...
  CvMat invPredObsCovariance =
      cvMat(obs_n, obs_n, CV_64F, this->invPredObsCovariance->data.db);
