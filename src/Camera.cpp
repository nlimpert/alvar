...
bool Camera::LoadCalibXML(const char *calibfile) {
  ...
  return xml_root->QueryIntAttribute("width", &calib_x_res) == TIXML_SUCCESS &&
         ...
         FileFormatUtils::parseXMLMatrix(
             xml_root->FirstChildElement("intrinsic_matrix"), &calib_K) &&
         FileFormatUtils::parseXMLMatrix(
             xml_root->FirstChildElement("distortion"), &calib_D);
}
...
void Camera::Undistort(vector<PointDouble> &points) {
  ...
  for (unsigned int i = 0; i < points.size(); i++) {
    // compensate distortion iteratively
    double x = (points[i].x - cx) * ifx, y = (points[i].y - cy) * ify, x0 = x,
           y0 = y;
    for (int j = 0; j < 5; j++) {
      double r2 = x * x + y * y;
      double icdist = 1. / (1 + k[0] * r2 + k[1] * r2 * r2);
...
/*
        ...
        void Undistort(PointType& point) {
                ...
                // compensate distortion iteratively
                double x = (point.x - cx)*ifx, y = (point.y - cy)*ify, x0 = x,
   y0 = y; for(int j = 0; j < 5; j++){ double r2 = x*x + y*y; double icdist
...
void Camera::CalcExteriorOrientation(vector<CvPoint3D64f> &pw,
  ...
  // cvmodFindExtrinsicCameraParams2(object_points, image_points, &calib_K,
...
Homography::Homography() { cvInitMatHeader(&H, 3, 3, CV_64F, H_data); }
