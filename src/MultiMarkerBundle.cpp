...
bool MultiMarkerBundle::Optimize(Camera *_cam, double stop, int max_iter,
  ...
  for (size_t f = 0; f < frames; f++) {
    ...
    for (size_t i = 0; i < marker_indices.size(); ++i) {
      int id = marker_indices[i];
      if (measurements.find(measurements_index(f, id, 0)) !=
          measurements.end()) {
        for (int j = 0; j < 4; j++) {
  ...
  if ((frames > 3) && (optimization_error > stop)) {
          ...
          if (max_k >= 0) {
                  ...
                  cout<<"Optimization error over the threshold -- remove the
  biggest outlier: "; cout<<"frame "<<f<<" id "<<id<<endl;
                  ...
                  for (int j=0; j<4; j++) {
                          ...
                          cvmSet(measurements_mat, index+0, 0,
  measurements[measurements_index(f, id, j)].x); cvmSet(measurements_mat,
  index+1, 0, measurements[measurements_index(f, id, j)].y);
                  }
