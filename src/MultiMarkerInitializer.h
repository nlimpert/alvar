...
protected:
  ...
  typedef std::vector<std::vector<MarkerMeasurement>>::iterator
      MeasurementIterator;
  FilterMedian *pointcloud_filtered;
  int filter_buffer_min;

  bool updateMarkerPoses(std::vector<MarkerMeasurement> &markers,
                         const Pose &pose);
...
public:
   ...
   */
  template <class M> void MeasurementsAdd(const std::vector<M> *markers) {
