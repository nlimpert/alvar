...
void ALVAR_EXPORT DrawLine(IplImage *image, const Line line,
                           CvScalar color = cvScalar(CV_RGB(0, 255, 0)));
...
void ALVAR_EXPORT DrawPoints(IplImage *image, const CvSeq *contour,
                             CvScalar color = cvScalar(CV_RGB(255, 0, 0)));
 ...
 * Controur sequence. \param radius	Circle radius in pixels. \param color
 * Use CV_RGB(red,green,blue) to determine the color.
 */
 ...
 * example implementation. \param image		Pointer to the original video
 * frame from where the hiding texture is calculated. \param hide_texture
 * Pointer to the destination image where the resulting texture is stored.
 * \param cam			Camera object that is used for marker tracking.
 * \param gl_modelview	Current model view matrix.
 * \param topleft		Top left limit of the texture area in marker
 * coordinate frame.
 ...
 * SampleMarkerHide.cpp for example implementation.
 *  \param image		Pointer to the destination image where the texture is
 * drawn. \param texure		Pointer to the texture image genereated by \e
 * BuildHideTexture.
 *  \param cam			Camera object that is used for marker tracking.
 *  \param gl_modelview	Current model view matrix.
 *  \param topleft		Top left limit of the texture area in marker
