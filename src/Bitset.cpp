...
int Bitset::Length() { return bits.size(); }
...
void Bitset::flip(size_t pos) { bits[pos] = !bits[pos]; }
...
int BitsetExt::hamming_dec_block(unsigned long block_len,
  ...
  if (block_len == (next_parity >> 1)) {
    parity = parity & ~(next_parity >>
                        1); // The last parity bit shouldn't be included in the
                            // other parity tests (TODO: Better solution)
    if (total_parity == 0) {
...
BitsetExt::BitsetExt() { SetVerbose(false); }
BitsetExt::BitsetExt(bool _verbose) { SetVerbose(_verbose); }
/*
void BitsetExt::str_8to6bit() {
        // TODO: Assume that the bitset contains 8-bit ASCII chars and change
them into 6-bit chars
}
void BitsetExt::str_6to8bit() {
        // TODO: Assume that the bitset contains 6-bit chars and change them
into 8-bit ASCII chars
}
...
*/
void BitsetExt::SetVerbose(bool _verbose) { verbose = _verbose; }
