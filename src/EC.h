 ...
 */
template <typename T> class DoHandleTest {
protected:
  int type_id;
  bool needs_has_p2d;
  bool needs_has_p3d;
...
public:
  DoHandleTest(int _type_id = -1, bool _needs_has_p2d = false,
      ...
      : type_id(_type_id), needs_has_p2d(_needs_has_p2d),
 ...
 */
template <typename T> class DoEraseTest {
protected:
  int type_id;
...
public:
  DoEraseTest(bool _erase_without_p2d, bool _erase_without_p3d,
              int _type_id = -1)
      : type_id(_type_id), erase_without_p2d(_erase_without_p2d),
        erase_without_p3d(_erase_without_p3d), test_reprojection(false),
        limit_sq(0.f) {}
  DoEraseTest(float _limit, bool _erase_without_p2d = false,
              bool _erase_without_p3d = false, int _type_id = -1)
      : type_id(_type_id), erase_without_p2d(_erase_without_p2d),
        erase_without_p3d(_erase_without_p3d), test_reprojection(true),
...
public:
  ...
  do_handle_test, int type_id=0, int first_id=0, int last_id=65535)
  {
          ...
          for (;iter != iter_end; iter++) {
                  ...
                  f.has_p2d = false; // This is updated again to true if
  tracking succeeds features[feature_count] = f.p2d; ids[feature_count] =
  iter->first; feature_count++; if (feature_count == max_features) break;
          }
          // Check that next_id is ok
          if (next_id < first_id)	next_id = first_id;
          if (next_id > last_id) return false; // TODO: Make some better
  solution for this
          // Purge if needed
          if (purge) {
...
public:
   ...
   * type_id */
  template <typename T>
  bool UpdateRotation(std::map<int, T> &container, Pose *pose,
                      int type_id = -1) {
    DoHandleTest<T> do_handle_test_default(type_id);
...
template <class M> class MarkerDetectorEC : public MarkerDetector<M> {
protected:
  template <typename T>
  bool PreDetect(std::pair<const int, T> &p, int type_id) {
    return PreDetect(p.second, type_id);
  }
  template <typename T> bool PreDetect(T &p, int type_id) {
...
public:
  int GetId(int marker_id, int corner_id, int first_id = 0,
            int last_id = 65535) {
