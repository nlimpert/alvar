...
int IntIndex::get() const { return index; }
int IntIndex::get_next_step() const { return next_step; }
int IntIndex::end() const { return res; }

IntegralImage::IntegralImage() { sum = 0; }
...
Donahue1992 double a4 = cvGet2D(gray, j, i+1).val[0]; double a3 = cvGet2D(gray,
j, i).val[0]; double a2 = cvGet2D(gray, j+1, i).val[0]; double a1 =
cvGet2D(gray, j+1, i+1).val[0];
// Normal vectors;
...
void IntegralGradient::GetAveGradient(CvRect &rect, double *dirx,
                                      double *diry) {
  int count = 1;
