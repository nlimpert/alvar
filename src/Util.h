 ...
 */
template <class C> inline int ALVAR_EXPORT Sign(const C &v) {
  return (v < 0 ? -1 : 1);
}
 ...
 */
template <class C> inline double ALVAR_EXPORT Rad2Deg(const C &v) {
  return v * (180 / PI);
}
 ...
 */
template <class C> inline double ALVAR_EXPORT Deg2Rad(const C &v) {
  return v * (PI / 180);
}

/**
 * \brief Simple \e Point class meant to be inherited from OpenCV point-classes.
 * For example: Point<cv::Point2d> p
 */
template <class C, class D = int> struct ALVAR_EXPORT Point : public C {
  /**
 ...
 * in list PointList \brief  Returns distance
 * \param  index	index of point A in pointlist, where A is the starting
 * point of the closest polygon segment \param  isClosedPolygon is true if
 * polygon is closed (segment of the first and last point is also checked)
 */
double ALVAR_EXPORT polyLinePointDist(const std::vector<cv::Point> &points,
                                      cv::Point &C, int *index,
                                      int isClosedPolygon);
 ...
 * \param v		Sequence of numbers from where the zero crossings are
 * found. \param corners	Resulting index list of zero crossings. \param
 * offs \return			Number of zero crossings found.
 */
 ...
 * \return			Resulting value that is between \e min_val and
 * \e max_val.
 */
...
public:
  ...
  template <class C> Serialization &operator<<(C &serializable) {
    input = false;
   ...
   */
  template <class C> bool SerializeClass(C &serializable) {
