...
void LabelingCvSeq::LabelSquares(IplImage *image, bool visualize) {
  ...
  while (contours) {
    ...
    if (result->total == 4 &&
        ...
        fabs(cvContourArea(result, CV_WHOLE_SEQ)) >
            _min_area && // TODO check limits
        cvCheckContourConvexity(
            result)) // ttehop: Changed to 'contours' instead of 'result'
    {
...
template <class T> inline T absdiff(T c1, T c2) {
...
void FitLineGray(CvMat *line_data, float params[4], IplImage *gray) {
  ...
  */
  ...
  for (int l = 0; l < line_data->cols; l++) {
    CvPoint2D32f *p = (CvPoint2D32f *)CV_MAT_ELEM_PTR_FAST(
        *line_data, 0, l, sizeof(CvPoint2D32f));
