...
void SimpleSfM::Clear() { container.clear(); }
    ...
    for (iter = container.begin(); iter != iter_end; iter++) {
      ...
      if (!f.has_p3d && f.has_stored_pose && f.estimation_type < 2) {
        ...
        if (dist_sh_sq > parallax_length_sq) {
          ...
          if (cam.ReconstructFeature(&f.pose1, &pose, &u1, &u2, &f.p3d,
                                     10e-6)) {
            CreateShadowPoint(f.p3d_sh, f.p3d, &cam, &pose, parallax_length,
                              triangulate_angle);
            f.has_p3d = true;
...
bool SimpleSfM::UpdateRotationsOnly(IplImage *image) {
  ...
  } else // if(pose_ok) // Tracking going on...
  {
    ...
    if (pose_ok) {
      ...
      CvMat rot =
          cvMat(4, 4, CV_64F,
                rot_mat); // Rotation matrix (difference) from the tracker
...
void SimpleSfM::Draw(IplImage *rgba) {
  ...
  for (; iter != iter_end; iter++) {
    ...
    if (f.has_p2d) {
      ...
      //	cvLine(rgba, cvPointFrom32f(f.p2d),
      //cvPointFrom32f(f.projected_p2d), CV_RGB(255,0,255));
      //}
      // if (f.type_id == 0) {
      //	int id = iter->first;
      ...
      //	if (f.has_p3d) {
      //		cvCircle(rgba, cvPointFrom32f(f.p2d), 5,
      //CV_RGB(0,255,0));
      //	}
