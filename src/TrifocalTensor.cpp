  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      for (int k = 0; k < 3; k++) {
        ...
        T[i][j][k] = (data_p12[j][i] * data_p13[k][3]) -
                     (data_p12[j][3] * data_p13[k][i]);
      }
}

double *getRow(double *m, int row) { return &m[4 * row]; }
    ...
    for (int j = 0; j < 3; j++)
      for (int k = 0; k < 3; k++) {
        ...
        T[i][j][k] = sign * det(getRow(data_p0, (i + 1) % 3),
                                ...
                                getRow(data_p1, j), getRow(data_p2, k));
      }
}

double TrifocalTensor::projectAxis(const CvPoint2D64f &p0,
                                   const CvPoint2D64f &p1, int l) {
...
void TrifocalTensor::project(const CvPoint2D64f &p0, const CvPoint2D64f &p1,
                             CvPoint2D64f &p2) {
