...
double angle(cv::Point &A, cv::Point &B, cv::Point &C, cv::Point &D,
...
double polyLinePointDist(const std::vector<cv::Point>& points,const cv::Point &C, int *index,
                         int isClosedPolygon) {
  //	Calculate minimum distance of Point C to Polygon whose points are in
  //list PointList 	if isClosedPolygon is true polygon is closed (segnment of the
...
int find_zero_crossings(const vector<double> &v, vector<int> &corners,
                        int offs) {
  int ind = 0;
  int len = (int)v.size();
...
void Histogram::AddDimension(int binsize) { dim_binsize.push_back(binsize); }
...
bool Serialization::Output() {
  SerializationFormatterXml *xml =
      (SerializationFormatterXml *)formatter_handle;
  if (filename.size() > 0) {
...
bool Serialization::Input() {
  ...
  if (filename.size() > 0) {
    ...
    if (node == 0) {
      node =
          (TiXmlElement *)xml->document.LinkEndChild(new TiXmlElement("root"));
    }
